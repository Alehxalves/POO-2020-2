...
public class WhatsappService{
    ...
    public void createChatGroup(String userId, String chatId){
        User user = getUser(userId);
        if(chatExist(chatId)){
            throw new RuntimeException("fail: group " + chatId + " ja existe");
        }
        rep_chat.put(chatId, new Group(chatId, user));
    }
    public void createChatTalk(String userOne, String userTwo){
        User user1 = getUser(userOne);
        User user2 = getUser(userTwo);
        TreeSet<String> ordenar = new TreeSet<>();
        ordenar.add(userOne);ordenar.add(userTwo);
        String chatId = ordenar.first() + "-" + ordenar.last();  
        if(chatExist(chatId)){
            throw new RuntimeException("fail: chatTalk " + chatId + " ja existe");
        }
        rep_chat.put(chatId, new Talk(user1, user2, chatId));
    }
    public void addByInvite(String inviteId, String guestId, String chatId){
        User invite = getUser(inviteId);
        User guest = getUser(guestId);
        Chat chat = getChat(chatId);
        chat.addByInvite(invite, guest);
        System.out.println(guestId + " juntou-se ao group " + chatId);
    }
    ...
    public String getHistory(String userId, String chatId){
        User user = getUser(userId);
        Chat chat = getChat(chatId);
        List<Msg> history = chat.getHistorico(user);
        StringBuilder saida = new StringBuilder();
        for(Msg msg : history)
